server <- function(input, output) {
  ...
  output$atentados_por_tipo_ataque <- renderPlotly({
    ...
    gg <- ggplot(data = atentados_por_tipo, 
      ...
      ## Define barra do grafico
      geom_bar(stat = 'identity',
               aes(fill = tipo_ataque),
               width = 0.8) +
      
      ## Rotaciona grafico de barras para melhorar a legibilidade
      coord_flip() +
  ...
  output$atividades_grupos_mais_atuantes <- renderPlotly({
    ...
    ## Gera data frame com os dados agrupados por organizacao
    grupos <- grupos %>%
      group_by(organizacao_terrorista) %>%
      summarise(atentados = n()) %>%
      top_n(n = 10, wt = atentados)
    
    ## Gera data frame com os dados agrupados por ano e organizacao
